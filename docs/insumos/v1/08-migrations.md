# 08 — Migrations (Prévia SQL)

> **ATENÇÃO:** Este arquivo é uma **prévia/referência** para a migration real.
> A criação efetiva do arquivo SQL **deve usar a skill `sql-migration`** conforme as regras do projeto.

---

## SQL Completo (Referência)
...
INSERT INTO `supply_categories` (`name`, `description`, `sort_order`) VALUES
    ('Matéria-Prima', 'Materiais base para produção', 1),
    ('Embalagem', 'Materiais de embalagem e acondicionamento', 2),
    ('Acabamento', 'Materiais de acabamento e finalização', 3),
    ('Fixação', 'Parafusos, pregos, cola, adesivos', 4),
    ('Químico', 'Tintas, solventes, vernizes, tratamentos', 5),
    ('Consumível', 'Materiais de consumo interno (lixas, estopas, etc.)', 6);
```

---

## Ordem de Execução das Tabelas

A ordem respeita dependências de FK:

1. `supply_categories` (sem dependências)
2. `supplies` (depende de `supply_categories`)
3. `supply_suppliers` (depende de `supplies` e `suppliers`)
...
| Tabela | Módulo | Já Existe |
|--------|--------|-----------|
| `suppliers` | Fornecedores | ✅ Sim |
| `products` | Produtos | ✅ Sim |
| `warehouses` | Estoque | ✅ Sim |
| `users` | Usuários | ✅ Sim |
...
```sql
-- ⚠ SOMENTE EM CASO DE ROLLBACK — NÃO EXECUTAR EM PRODUÇÃO SEM CONFIRMAÇÃO
DROP TABLE IF EXISTS `supply_stock_movements`;
DROP TABLE IF EXISTS `supply_stock_items`;
