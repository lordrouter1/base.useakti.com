...
## Status de Implementação (atualizado em 2026-04-16)

| Fase | Status | Itens Feitos | Itens Pendentes | Observações |
|------|--------|-------------|-----------------|-------------|
| **F0** | ✅ Completa | 18/18 | 0 | Pré-existente no v1 |
| **F1** | ⚠️ 90% | 10/13 | 3 (F1.3.3-F1.3.5 UI variação na BOM, F1.4 testes) | Service e migration OK |
| **F2** | ⚠️ 85% | 13/16 | 3 (F2.1.5 sugestão FEFO na UI, F2.2.4 conversão fornecedor, F2.3.5 auto-check, F2.4.2-F2.4.3 cards, F2.5 testes) | FEFO+CMP core OK |
| **F3** | ⚠️ 90% | 12/14 | 2 (F3.2.8 forecast no SupplyStockController, F3.4 testes) | Service+model+views OK |
| **F4** | ⚠️ 85% | 13/15 | 2 (F4.1.7 substituição automática, F4.2.4-F4.2.5, F4.4 testes) | Dashboard completo |
| **F5** | ❌ Não iniciada | 0/11 | 11 | Testes, QA, docs |

**Itens críticos pendentes:**
- ❌ `forecast()` e `recalculateForecast()` declarados em routes mas não implementados no `SupplyStockController`
- ❌ `costAlerts()` declarado em routes mas não implementado no `SupplyController`
- ❌ Nenhum teste PHPUnit criado (F1.4, F2.5, F3.4, F4.4, F5.1-F5.2)
- ⚠️ F4.2.5: baixa adicional quando real > planejado não implementada
- ⚠️ F2.3.5: `checkMarginImpact()` não integrado ao fluxo de entrada automático
...
**Critérios de Aceite:**
- [x] Entrada cria `supply_stock_items` (ou atualiza) + registra `supply_stock_movements`
- [x] Saída valida estoque e registra movimento
- [x] Transferência cria 2 movimentos atomicamente
- [x] Ajuste registra com reason
- [x] Todos os movimentos têm `created_by`
...
**Dependências:** Nenhuma
**Critérios de Aceite:**
- [x] AJAX: listar, adicionar, editar, remover fornecedores de um insumo
- [x] Fator de conversão exibido e editável
- [x] Apenas 1 fornecedor preferido por insumo
- [x] Select2 para busca de fornecedores
...
**Dependências:** Nenhuma
**Critérios de Aceite:**
- [x] AJAX: adicionar/editar/remover insumos de um produto
- [x] Cálculo de custo total de BOM exibido na tela
- [x] "Onde é Usado" exibe lista de produtos que usam o insumo
- [x] Histórico de preços com dados
...
**Critérios de Aceite:**
- [x] Histórico de movimentações filtrado por tipo, período, insumo, depósito
...
**Dependências:** Nenhuma
**Critérios de Aceite:**
- [x] Arquivo SQL gerado em `/sql/` com nomenclatura correta
- [x] Colunas com defaults sensatos (permite_fracionamento=1, decimal_precision=4, loss_percent=0)
...
**Critérios de Aceite:**
- [x] `calculateEffectiveQuantity()` respeita `permite_fracionamento` (CEIL para não fracionáveis)
- [x] `calculateBomForLot()` herda insumos do pai e faz override por variação
- [x] `calculateProductionCost()` soma custo de todos insumos obrigatórios
- [x] `checkAvailability()` retorna lista de insumos faltantes
...
**Critérios de Aceite:**
- [x] Checkbox funcional com tooltip explicativo
...
**Critérios de Aceite:**
- [x] Saída consome lote com menor `expiry_date` primeiro
- [x] Consumo parcial de lotes funciona corretamente
...
**Critérios de Aceite:**
- [x] Fórmula CMP correta com 4 casas decimais
- [ ] Conversão de fornecedor aplicada antes do CMP
- [x] Histórico de preço registrado com source = 'cmp_calculado'
...
- [x] Preço sugerido calculado corretamente
- [x] Ação "Aplicar" atualiza preço do produto
- [x] Configuração de margem mínima por tenant
...
- [x] Ordenação por urgência (mais próximo primeiro)
...
**Critérios de Aceite:**
- [x] CRUD via AJAX com SweetAlert2
- [x] Busca de substituto por prioridade + estoque
- [x] Conversão de taxa aplicada corretamente
...
| F3.2.7 | ⚠️ Criar partial `supply_stock/_forecast_detail.php` — *detalhe inline na view via SweetAlert2 modal* | View |
| F3.2.8 | ❌ Adicionar action `forecast()` no `SupplyStockController` — *rota declarada mas método não implementado* | Controller |
| F3.2.9 | ✅ Adicionar rota e menu | Config |

**Dependências:** F1.2 (InsumoService para BOM), F0.4 (stock model)
**Critérios de Aceite:**
- [x] Dashboard com KPIs (ruptured, critical, warning, ok)
- [x] Tabela filtrável com todos os insumos
- [x] Detalhe mostra pedidos que demandam + substitutos
...
**Critérios de Aceite:**
- [x] Quando ruptura, sistema mostra substituto com estoque e taxa de conversão
- [x] Consumo de produção oferece substituição automática
...
| F4.1.6 | ✅ Lógica de fracionamento (CEIL) no consumo — *via InsumoService.calculateEffectiveQuantity()* | Service |
| F4.1.7 | ⚠️ Lógica de substituição parcial no consumo — *substituto sugerido mas não aplicado automaticamente no consumo* | Service |

**Dependências:** F1.2 (InsumoService), F2.1 (FEFO), F3.1 (substitutos)
**Critérios de Aceite:**
- [x] Consumo calcula BOM completa para o lote
- [x] FEFO aplicado na seleção de lotes
- [x] Fracionamento (CEIL) aplicado corretamente
- [ ] Substituição oferecida quando necessário
- [x] Log de consumo planejado registrado
...
| F4.2.5 | ⚠️ Se real > planejado, ajustar estoque (baixa adicional) — *não implementado* | Service |
| F4.2.6 | ✅ Se real < planejado, devolver ao estoque (ajuste positivo) | Service |

**Dependências:** F4.1
**Critérios de Aceite:**
- [x] Operador preenche consumo real por insumo
- [x] Variância calculada automaticamente
- [ ] Ajuste de estoque automático na confirmação (parcial: apenas devolução implementada)
- [x] Registro de audit trail
...
**Critérios de Aceite:**
- [x] Gráfico renderiza dados reais
- [x] Filtros por período e produto funcionando
...
- [x] Código segue PSR-4, namespace `Akti\`
- [x] Prepared statements em todas as queries
- [x] Escape `e()` em todas as views
- [x] `csrf_field()` em todos os forms
- [x] `X-CSRF-TOKEN` em todos os AJAX
- [x] SweetAlert2 para confirmações (nunca `alert()`/`confirm()`)
- [x] Responsivo (testado em mobile)
- [ ] Testes PHPUnit passando
- [x] Migration SQL gerada via skill `sql-migration`
