...
$(document).ready(function() {
    ...
    function loadReposAsync() {
        $.getJSON('?page=git&action=loadRepos', function(data) {
            if (!data.success) {
    ...
    function renderRepos(repos) {
        ...
        repos.forEach(function(repo) {
            ...
            html += '<div class="col-xl-6 repo-card-item" data-repo-name="' + escapeHtml(repo.name) + '" data-status="' + status + '">';
            ...
            html += '<button class="btn btn-sm btn-outline-secondary btn-fetch" data-repo="' + escapeHtml(repo.name) + '" title="Fetch"><i class="fas fa-sync"></i></button>';
            html += '<button class="btn btn-sm btn-outline-primary btn-pull" data-repo="' + escapeHtml(repo.name) + '" data-name="' + escapeHtml(repo.name) + '" title="Pull"><i class="fas fa-cloud-arrow-down"></i></button>';
            html += '<button class="btn btn-sm btn-outline-info btn-detail" data-repo="' + escapeHtml(repo.name) + '" title="Ver detalhes"><i class="fas fa-info-circle"></i></button>';
            html += '<button class="btn btn-sm btn-outline-danger btn-force-reset" data-repo="' + escapeHtml(repo.name) + '" data-name="' + escapeHtml(repo.name) + '" title="Force Reset"><i class="fas fa-skull-crossbones"></i></button>';
            html += '</div></div>';
    ...
    $('#btnDiagnose').on('click', function() {
        ...
        $.getJSON('?page=git&action=diagnoseJson', function(data) {
    ...
    $('#btnFetchAll').on('click', function() {
        ...
        $.post('?page=git&action=fetchAll', function(data) {
    ...
    $('#btnPullAll').on('click', function() {
        ...
        Swal.fire({
            ...
            $.post('?page=git&action=pullAll', function(data) {
                Swal.close();
                if (data.success) {
    ...
    $(document).on('click', '.btn-fetch', function() {
        ...
        $.post('?page=git&action=fetch', {repo: repo}, function(data) {
    ...
    $(document).on('click', '.btn-pull', function() {
        ...
        Swal.fire({
            ...
            $.post('?page=git&action=pull', {repo: repo}, function(data) {
                ...
                if (data.needs_stash) {
                    ...
                    Swal.fire({
                        ...
                        if (res.isConfirmed) {
                            ...
                            $.post('?page=git&action=pull', {repo: repo, force_stash: 1}, function(d) {
                                if (d.success) {
    ...
    function doForceReset(repo, repoName) {
        Swal.fire({
            ...
            $.post('?page=git&action=forceReset', {repo: repo, confirmed: 1}, function(data) {
                if (data.success) {
    ...
    $(document).on('click', '.btn-detail', function() {
        ...
        $.getJSON('?page=git&action=detail&repo=' + encodeURIComponent(repo), function(data) {
            if (!data.success) {
