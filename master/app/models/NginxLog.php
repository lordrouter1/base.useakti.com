...
/**
 * Model: NginxLog
 * Lê e analisa logs de erro e acesso do Nginx
 *
 * Na VPS Debian:
 *   - Pasta de logs: /var/log/nginx/
 *   - O usuário do PHP (www-data) precisa de permissão de leitura
...
class NginxLog
{
    ...
    private static function getLogPath()
    {
        ...
        if (PHP_OS_FAMILY === 'Windows') {
            return 'd:/xampp/logs'; // Fallback local
        }
    ...
    public static function listLogFiles()
    {
        ...
        foreach ($entries as $entry) {
            ...
            // Filtrar apenas arquivos de log
        ...
        // Ordenar: errors primeiro, depois por data
     ...
     * Usa tail para eficiência (sem carregar o arquivo inteiro na memória)
     */
    public static function readTail($filename, $lines = 200)
    {
        ...
        $lines = min(max($lines, 10), 2000); // Limitar entre 10 e 2000
        ...
        if (PHP_OS_FAMILY !== 'Windows' && function_exists('exec')) {
            ...
            // Fallback: ler o arquivo do final
    ...
    /**
     * Lê tail de arquivo .gz
     */
    private static function readGzTail($path, $lines)
    {
        if (PHP_OS_FAMILY !== 'Windows' && function_exists('exec')) {
            $output = [];
    ...
    /**
     * Tail implementado em PHP puro (fallback)
     */
    private static function phpTail($path, $lines)
    {
        $fp = @fopen($path, 'rb');
        if (!$fp) return '';
     ...
     */
    public static function search($filename, $query, $maxResults = 100)
    {
    ...
    /**
     * Analisa os erros mais frequentes de um log de erro
     */
    public static function analyzeErrors($filename, $limit = 20)
    {
        ...
        foreach (explode("\n", $tail['content']) as $line) {
            ...
            // Extrair o tipo de erro (ex: "PHP Warning:", "PHP Fatal error:", etc.)
    ...
    /**
     * Retorna o caminho seguro de um arquivo de log para download
     */
    public static function getDownloadPath($filename)
    {
        ...
        // Segurança: garantir que está dentro da pasta de logs
