...
$(document).ready(function() {
    ...
    function loadLogContent(file, lines) {
        ...
        $('#logContentCard').show();
        $('#btnDownloadLog').attr('href', '?page=master_logs&action=download&file=' + encodeURIComponent(file)).show();

        $.getJSON('?page=master_logs&action=read', {file: file, lines: lines}, function(data) {
            if (data.success) {
    ...
    function searchLog(file, query) {
        ...
        $.getJSON('?page=master_logs&action=search', {file: file, q: query}, function(data) {
            if (data.success) {
...
<div class="card mb-4 border-info">
    <div class="card-body text-center py-5">
        ...
        <p class="text-muted mb-0">O sistema está rodando em uma base de testes.<br>O módulo de logs do servidor não está disponível neste ambiente.</p>
    </div>
</div>
<?php else: ?>
